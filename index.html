...
<div class="settings">
  ...
  <div class="setting-row">
    <span>⏳ 制限時間</span>
    <input type="number" id="timeMin" value="0" min="0" style="width:60px;"> 分
    <input type="number" id="timeSec" value="30" min="0" max="59" style="width:60px;"> 秒
  </div>
  ...
  <div class="pause-resume-container">
    ...
    <button onclick="resumeGame()">▶ 再スタート</button>
  </div>
</div>
...
<div class="footer">
残り時間：<span id="remain">0分0秒</span><br>
超過回数：<span id="count">0</span>
</div>

</div>

<div id="successOverlay" class="success-overlay">
  <div class="success-text">チャレンジ成功！</div>
  ...
  <button class="success-button" onclick="closeSuccess()">画面に戻る</button>
...
function showSuccess(){
  if(overHistory.length){
    document.getElementById("scoreHistory").innerHTML = "🔊 音量履歴:<br>" +
      overHistory.map(entry=>{
        const m=Math.floor(entry.time/60);
        const s=entry.time%60;
        return `${m}分${s}秒: ${entry.db} dB`;
      }).join("<br>");
  }else{
    document.getElementById("scoreHistory").textContent="🔊 音量履歴: なし";
  }

  document.getElementById("scoreSilent").textContent =
    "🤫 最長サイレント時間: "+longestSilent.toFixed(1)+" 秒";
...
function resetGame(){
  ...
  waveformData=[];
  document.getElementById("remain").textContent="0分0秒";
